    ...
    **References:**
    ...
    def run(self, dag):
        ...
        while front_layer:
            ...
            for swap_qubits in swap_scores:
                ...
                #"""sl: Record the average distance gain; the larger the better"""
                #num_solved_gates = self._compute_solved_gates(front_layer, trial_layout)
                #swap_scores[swap_qubits] = prescore - score + num_solved_gates
                #swap_scores[swap_qubits] = prescore - score
                #if num_solved_gates: 
                #    print(prescore, score, num_solved_gates)
                #swap_scores[swap_qubits] = prescore - score 
                swap_scores[swap_qubits] = score 
            ...
            """sl: Check if remote CNOT can be applied"""
            bridge_candidates = self._obtain_bridges(dag, front_layer, current_layout) #sl: a dict
            ...
            if UseBridge and bridge_candidates: 
                ...
                best_bridges = [bridge for bridge in bridge_candidates if bridge_candidates[bridge] == best_bridge_score]
    ...
    def _obtain_bridges(self, dag, front_layer, layout):
        ...
        for node in front_layer: 
            if len(node.qargs) != 2: 
                continue
            if self.dist_matrix[layout_map[node.qargs[0]], layout_map[node.qargs[1]]] == 2:
                solvable_successor = 0
                for suc in self._successors(node, dag):
                    if len(suc.qargs) == 2:
                        if self.dist_matrix[layout_map[suc.qargs[0]], layout_map[suc.qargs[1]]]==1:
    ...
    """sl: compulte the number of solved gates in the current layer under the current layout"""
    def _compute_solved_gates(self, layer, layout):
        cost = 0
        layout_map = layout._v2p
        for node in layer:
            if len(node.qargs) != 2:
                continue
            if self.dist_matrix[layout_map[node.qargs[0]], layout_map[node.qargs[1]]]==1:
                cost += 1
        return cost
