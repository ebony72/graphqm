...
#EXTENDED_SET_SIZE = 16  #sl Size of lookahead window. TODO: set dynamically to len(current_layout)
    ...
    **References:**
    ...
    def run(self, dag):
        ...
        while front_layer:
            ...
            """sl: Record the prescore when no front_layer gate is executable """
            prescore = self._score_heuristic(
                    self.heuristic, front_layer, extended_set, current_layout, swap_qubits=None
                )
            
            """sl: No solvable gates under the current layout; otherwise they should have been executed..""" 
            pre_solved_gates = self._compute_solvable_gates(dag, front_layer, extended_set, current_layout)
            if pre_solved_gates: 
                print(f'ha{pre_solved_gates}')
            #prescore += pre_solved_gates
            ...
            for swap_qubits in swap_scores:
                ...
                #sl: choice 2 (seems not better than choice 1)
                #num_solvable_gates = self._compute_solvable_gates(dag, front_layer, extended_set, trial_layout)
                "TODO: How to combine the two parts?"
                #swap_scores[swap_qubits] = score - num_solvable_gates
            

            best_score = min(swap_scores.values()) 

            """sl: Check if bridge gates can be applied"""
            bridge_candidates = self._obtain_bridges(dag, front_layer, extended_set, current_layout) #sl: a dict
            UseBridge = True #use bridge gate
            if UseBridge and bridge_candidates: 
                best_bridge_score = max(bridge_candidates.values()) #sl: take values from {0, 1, 2}
                best_bridges = [k for k, v in bridge_candidates.items() if v == best_bridge_score]

                #print('--------------------')
                #print(best_score, best_bridge_score, len(bridge_candidates))
            ...
            #print(f"the selected swap is {best_swap[0]._index, best_swap[1]._index}")
    ...
    #sl
    def _obtain_bridges(self, dag, front_layer, extended_set, layout):
        bridge_set = dict()
        for node in front_layer: 
            if len(node.qargs) != 2: 
                continue
            if self.dist_matrix[layout._v2p[node.qargs[0]], layout._v2p[node.qargs[1]]] == 2:
                ...
                #choice 1: (seems worse than choice 2)
                temp_front_layer = copy(front_layer)
                temp_front_layer.remove(node)
                solvable_successor = self._compute_solvable_gates(dag, temp_front_layer, extended_set, layout)
                
                #choice 2:
                solvable_successor = 0
                for suc in self._successors(node, dag):
                    if len(suc.qargs) == 2:
                        if self.dist_matrix[layout._v2p[suc.qargs[0]], layout._v2p[suc.qargs[1]]]==1:
                            solvable_successor += 1
                            
                bridge_set[node] = solvable_successor
        return bridge_set
    ...
    def _obtain_extended_set(self, dag, front_layer):
        ...
        while tmp_front_layer and not done:
            ...
            for node in tmp_front_layer:
                ...
                if len(extended_set) >= self.coupling_map.size():
                    done = True
                    break
    ...
    """TODO: compute the number of solved gates in the current layer under the current layout"""
    def _compute_solvable_gates(self, dag, front_layer, extended_set, layout):
        solvable_gates = []
        temp_applied_predecessors = copy(self.applied_predecessors)
        temp_front_layer = copy(front_layer)
        CHANGE = True
        while temp_front_layer and CHANGE:
            CHANGE = False
            for node in temp_front_layer:
                if node not in front_layer and node not in extended_set:
                    break
                if temp_applied_predecessors[node] < len(node.qargs):
                    continue
                if len(node.qargs) != 2 or self.dist_matrix[layout[node.qargs[0]], layout[node.qargs[1]]]==1:
                    #print(node.qargs, self.dist_matrix[layout[node.qargs[0]], layout[node.qargs[1]]])
                    CHANGE = True
                    if len(node.qargs) == 2:
                        solvable_gates.append(node)
                    temp_front_layer.remove(node)
                    for successor in self._successors(node, dag):
                        temp_applied_predecessors[successor] += 1
                        if temp_applied_predecessors[successor] == len(successor.qargs):
                            temp_front_layer.append(successor)
                            
        #if solvable_gates: 
        #    print(f'solvabe gates {len(solvable_gates)}')
        return len(solvable_gates)
