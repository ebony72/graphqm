...
class Vf:
    ...
    def dfsMatch(self): #sl stop is the time limit
        ...
        if self.is_over and len(self.result) != len(nx.nodes(self.subgraph)):
            print('chee')
            return self.result
        ...
        for v2 in gNMNeighbor:
            if self.isMeetRules(v1, v2):
                ...
                #     return self.result
                # elif self.is_over:
                #     print('Vf2 should have been failed!')
                #     return self.result
                # else:
                #     if self.is_over:
                #         print('why?')
        ...
        if len(self.result) == len(nx.nodes(self.subgraph)):
            print('check twice why we are here!', self.is_over)
            self.is_over = True
            return self.result        
        if result1 == self.result:
            print('the vertex is not extendable', v1, len(self.result))
